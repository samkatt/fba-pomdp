...
CollisionAvoidanceBigTablePrior::CollisionAvoidanceBigTablePrior(
    ...
    configurations::BAConf const& c) :
        ...
        _domain_size(
            static_cast<int>(_width * std::pow(_height, _num_obstacles + 1) * _num_speeds * _num_traffics * _num_timeofdays),
            _NUM_ACTIONS,
                    ...
                    for (auto speed = 0; speed < _num_speeds; ++speed) {
                        for (auto traffic = 0; traffic < _num_traffics; ++traffic) {
                            for (auto timeofday = 0; timeofday < _num_timeofdays; ++timeofday) {
                                auto const s = d.getState(x, y, speed, traffic, timeofday, obstacles);
    ...
    domains::CollisionAvoidanceBig const& d)
{
    ...
    auto const s      = d.getState(x, y, 0, 0, 0, obstacles); // TODO change if I need this
    auto const new_s  = d.getState(new_x, new_y, 0, 0, 0, new_obstacles); // TODO change if I need this
...
CollisionAvoidanceBigFactoredPrior::CollisionAvoidanceBigFactoredPrior(
    configurations::FBAConf const& conf) :
        ...
        _domain_size(
            _width * _height * _num_speeds * _num_traffics * _num_timeofdays * static_cast<int>(std::pow(_height, _num_obstacles)),
            _NUM_ACTIONS,
            static_cast<int>(std::pow(_height, _num_obstacles))),
        _domain_feature_size(
            std::vector<int>(5 + _num_obstacles, _height),
            ...
            for (auto y = 0; y < _height; ++y) {
                for (auto speed = 0; speed < _num_speeds; ++speed) {
                    model.transitionNode(&action, f)
                            .setDirichletDistribution({y, speed}, obstacleTransition(y, speed));
                }
            }
        }
    }
...
std::vector<float> CollisionAvoidanceBigFactoredPrior::obstacleTransition(int y) const
{
    ...
    // then it takes on 3 times as many probability
...
std::vector<float> CollisionAvoidanceBigFactoredPrior::timeofdayTransition(int timeofday) const {
    ...
    timeofday_counts[timeofday] += _counts_total;
